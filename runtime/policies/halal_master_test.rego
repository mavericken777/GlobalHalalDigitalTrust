...
test_nominal_clearance if {
	platinum.allow with input as nominal
	platinum.hti_score == 100.0 with input as nominal
}

test_shafii_violation if {
	bad := json.patch(nominal, [{"op": "replace", "path": "/madhab_scores/shafii", "value": 0.8}])
	not platinum.allow with input as bad
	platinum.hti_score == 0.0 with input as bad
}

test_cold_chain_quarantine if {
	hot := json.patch(nominal, [{"op": "replace", "path": "/telemetry/temp_celsius", "value": -14.0}])
	platinum.quarantine_required with input as hot
	platinum.hti_score == 0.0 with input as hot
}
